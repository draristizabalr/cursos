...
const expetedPokemons: SimplePokemon[] = [
  { id: '1', name: 'bulbasaur' },
  { id: '2', name: 'ivysaur' },
];
...
describe('PokemonsService', () => {
  ...
  it('should load a page of a SimplePokemons', () => {
    service.loadPage(1).subscribe((pokemons) => {
      expect(pokemons).toEqual(expetedPokemons);
    });
  ...
  it('should catch error if pokemon not found', () => {
    const name = 'not-pokemon-name'

    service.loadPokemon(name).pipe(
      catchError((error: HttpErrorResponse) => {
        expect(error.message).toContain('Pokémon not found');
        return [];
      })
    ...
    req.flush('Pokémon not found', { status: 404, statusText: 'Not found' });
  });
